...
const ArticlePageComponent = ({ article }: ArticlePageComponentProps) => {
                        ...
                        {article.map(item => (
                            ...
                            >
                                <Link href={`articles/${item.slug}`}>
                                    <Box pos={'absolute'} top={0} right={0} bottom={0} bg={'rgba(0, 0, 0, 0.7)'} />
            ...
            <Grid gridTemplateColumns={{ base: 'repeat(1, 1fr)', lg: 'repeat(2, 1fr)' }} gap={4} >
                {article.map(item => (
                    <Box key={item.id} w={'full'} bg={cardBackgroundColor} boxShadow={'2xl'} rounded={'md'} p={6} >
                        <Link href={`articles/${item.slug}`}>
                            <Image src={item.image.url} alt={item.title} w={'full'} h={'250px'} objectFit={'cover'} borderRadius={'lg'} mb={5} />
                            <Stack>
                                <Heading fontSize={'2xl'} fontFamily={'body'} >{item.title}</Heading>
                            ...
                            <Stack mt={6} direction={'row'} spacing={4} align={'center'} >
                                ...
                                <Stack direction={'column'} spacing={0} fontSize={'sm'}>
                                    ...
                                    <Text color={'gray.500'}>{format(new Date(item.createdAt), 'dd MMM, yyyy')} . {calculateEstimatedRedingTime(item.description.text)} {t('read_article', { ns: 'layout' })}</Text>
                                </Stack>
                            </Stack>
                        </Link>
